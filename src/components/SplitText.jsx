...
const SplitText = ({
  ...
  useGSAP(
    () => {
      ...
      const splitHTML = words.map((word, wordIdx) => {
        const chars = word.split('').map((char, charIdx) => {
          return `<span class="split-char" style="display: inline-block;">${char}</span>`;
        }).join('');
        const space = wordIdx < words.length - 1 ? '<span class="split-space"> </span>' : '';
        return `<span class="split-word" style="display: inline-block; white-space: nowrap;">${chars}</span>${space}`;
      }).join('');
